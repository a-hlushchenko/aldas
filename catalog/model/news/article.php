...
class ModelNewsArticle extends Model {
	...
	public function getArticles($data = array()) {
		...
		// убираем store_id
		$sql .= " LEFT JOIN " . DB_PREFIX . "news_articles_description pd ON (p.article_id = pd.article_id) LEFT JOIN " . DB_PREFIX . "news_articles_to_store p2s ON (p.article_id = p2s.article_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1'";
	...
	public function getTotalArticles() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "news_articles WHERE status = '1'");
	...
	public function getArticleCarousel($category_id,$limit=4) {

		$article_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_articles_to_category a2c LEFT JOIN " . DB_PREFIX . "news_articles_description ad ON (a2c.article_id = ad.article_id) LEFT JOIN " . DB_PREFIX . "news_articles a ON (ad.article_id = a.article_id) WHERE category_id = '" . (int)$category_id . "' ORDER BY a.date_added DESC LIMIT ".(int)$limit);
