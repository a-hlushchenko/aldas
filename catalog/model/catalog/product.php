...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		// оптимизация убрал manufacturer, discount, special, reward,stock_status,weight_class,length_class,rating,reviews
		//$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, p.sort_order FROM " . DB_PREFIX . "product p 
	...
	public function getProducts($data = array()) {
		...
		//оптимизация запроса убрал rating, discount, special,
		...
		if (!empty($data['filter_category_id'])) {
			...
			$sql .= " FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id)";
		}
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			...
			if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
				//$sql .= " ORDER BY p.new DESC, LCASE(" . $data['sort'] . ")";
				$sql .= " ORDER BY LCASE(" . $data['sort'] . ")";
			} elseif ($data['sort'] == 'p.price') {
				 //убрал rating, discount, special,
				//$sql .= " ORDER BY (CASE WHEN new IS NOT NULL THEN new WHEN special IS NOT NULL THEN special WHEN discount IS NOT NULL THEN discount ELSE p.price END)";
				$sql .= " ORDER BY p.price)";
			} else {
				//$sql .= " ORDER BY p.new DESC, " . $data['sort'];
				$sql .= " ORDER BY " . $data['sort'];
			}
		} elseif(isset($data['sort']) && $data['sort']=='RAND()') {
			$sql .= " ORDER BY RAND()";
		} else {
			//$sql .= " ORDER BY p.new DESC, p.sort_order";
            if (!empty($data['filter_sub_category'])) {
				//$sql .= " ORDER BY p.new DESC, p2c.sort_order";
				$sql .= " ORDER BY p2c.sort_order";
			} else {
				//$sql .= " ORDER BY p.new DESC, p.sort_order";
				$sql .= " ORDER BY p.sort_order";
			}
			//$sql .= " ORDER BY (CASE WHEN new IS NOT NULL THEN new ELSE p.price END)";
		}
	...
	public function getProductCategories($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category ptc LEFT JOIN " . DB_PREFIX . "category c ON (ptc.category_id = c.category_id) WHERE ptc.product_id = '" . (int)$product_id . "' AND c.view_attibute = 1");
	...
	public function getProductsForCarousel($data) {
		
		$cache = new Cache('file',10800);//3часа

		$cache_data = $cache->get('ForCarouselHome_cat-'.$data['filter_category_id']);

		if (!$cache_data) {
			$product_data = array();

			$query = $this->db->query("SELECT p.product_id,p.image,p.price FROM " . DB_PREFIX . "product p 
				LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) 
				WHERE p2c.category_id = '".$data['filter_category_id']."' AND status = '1' 
				ORDER BY p2c.sort_order LIMIT " . (int)$data['limit_cache']);

			$cache_data = $query->rows;
			$cache->set('ForCarouselHome_cat-'.$data['filter_category_id'], $query->rows);
		}
