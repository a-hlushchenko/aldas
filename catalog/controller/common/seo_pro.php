		...
		switch ($route) {
			...
			case 'news/article':
				if (isset($data['article_id'])) {
					...
					$data['news_path'] = $this->getPathByArticle($tmp['article_id']);
					
					if (!$data['news_path']) return $link;
...
echo "</pre>";*/
	...
	private function getPathByArticle($article_id) {
		...
		static $news_path = null;
		if (!isset($news_path)) {
			$news_path = $this->cache->get('article.seopath');
			if (!isset($news_path)) $news_path = array();
		}

		if (!isset($news_path[$article_id])) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_articles_to_category");
			
			$news_path = array();
			foreach ($query->rows as $key => $row) {
				$news_path[$row['article_id']] = $row['category_id'];
			}
			$news_path[$article_id] = $query->num_rows ? (int)$query->row['category_id'] : 0;

			$this->cache->set('article.seopath', $news_path);
		}

		return $news_path[$article_id];
	}
