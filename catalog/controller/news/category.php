	...
	public function index() {
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				$results_image = $this->model_news_article->getArticleImages($result['article_id']);
				$c=0;
				foreach ($results_image as $im) {
					$c++;
					if($c>=4)break;
					$images[] = array(
				...
				$description_tmp = str_replace('&nbsp;', '', $description);
				$description_tmp = str_replace(' ', '', $description_tmp);
				if($description_tmp !=''){
			...
			$data['sorts'][] = array(
				...
				'href'  => $this->url->link('news/category', 'news_path=' . $this->request->get['news_path'] . '&sort=p.sort_order&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'pd.name-ASC',
				'href'  => $this->url->link('news/category', 'news_path=' . $this->request->get['news_path'] . '&sort=pd.name&order=ASC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'pd.name-DESC',
				'href'  => $this->url->link('news/category', 'news_path=' . $this->request->get['news_path'] . '&sort=pd.name&order=DESC' . $url)
			);
			...
			if ($this->config->get('news_review_status')) {
				$data['sorts'][] = array(
					...
					'value' => 'rating-DESC',
					'href'  => $this->url->link('news/category', 'news_path=' . $this->request->get['news_path'] . '&sort=rating&order=DESC' . $url)
				);

				$data['sorts'][] = array(
					'text'  => $this->language->get('text_rating_asc'),
					'value' => 'rating-ASC',
					'href'  => $this->url->link('news/category', 'news_path=' . $this->request->get['news_path'] . '&sort=rating&order=ASC' . $url)
				);
			}
			...
			$limits = array_unique(array($this->config->get('news_catalog_limit'), 5, 15, 25, 50));
			...
			foreach($limits as $value) {
				$data['limits'][] = array(
					'text'  => $value,
					'value' => $value,
					'href'  => $this->url->link('news/category', 'news_path=' . $this->request->get['news_path'] . $url . '&limit=' . $value)
				);
			}
