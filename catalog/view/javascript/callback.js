...
// шапка Заказать звонок
  ...
  $('#modal-agree .send').live('click', function() {

    var node = this;

    name = $('.modal-body input[name="name"]').val();
    tel = $('.modal-body input[name="tel"]').val();
    email = $('.modal-body input[name="email"]').val();
    comment = $('.modal-body textarea').val();

    $('.modal-body input, .modal-body textarea').attr('disabled', true);

    f = $('.modal-body .left').html();
    f += '<br/>Имя: '+name;
    f += '<br/>Телефон: '+tel;
    f += '<br/>E-mail: '+email;
    ...
    if(block_button) return false;
    $.ajax({
      url: 'index.php?route=information/callback/send4',
      type: 'post',
      data: 'text='+f,
      dataType: 'json',
      beforeSend: function() {
      ...
      success: function(json) {
        //console.log(json);
        $('#modal-agree .catalog-button').before('<div class="clear">'+json['success']+'</div>');
        $('#modal-agree .catalog-button').remove();
        //yaCounter39342560.reachGoal('tov_1click');//return true;
        //$('#modal-agree .catalog-button').html('<div success>'+data+'</div>');
      },
      error: function(xhr, ajaxOptions, thrownError) {
        alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
      }
