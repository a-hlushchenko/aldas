 ...
 */
 ...
 require_once DIR_SYSTEM . 'welldone/classes/minify/jsmin.php';
 require_once DIR_SYSTEM . 'welldone/classes/minify/css/Minify.php';
 require_once DIR_SYSTEM . 'welldone/classes/minify/css/CSS.php';
 require_once DIR_SYSTEM . 'welldone/classes/minify/css/Converter.php';
 
 class WelldoneMinifier
 {
   private $path;
   
   public function __construct() 
   {
     $this->path = DIR_SYSTEM.WELLDONE_CACHE_DIR.'minify/';
   }
     ...
     if (!file_exists($this->path . $combined_css_file)) 
     {
       foreach ($files as $style) 
       {
         ...
         if (!file_exists($file)) 
         {
           ...
           $minifier->minify('system/'.WELLDONE_CACHE_DIR.'minify/'.$file);  
           //$minifier->gzip('system/'.WELLDONE_CACHE_DIR.'minify/'.$file);  
       ...
       foreach ($files as $style) 
       {
          $file = 'system/'.WELLDONE_CACHE_DIR.'minify/'.$this->get_file_name($style, 'css');
          if (!file_exists($file)) 
          {
              continue;
          }
