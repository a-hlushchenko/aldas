...
if($user->isLogged()&& $user->getUserName()=='gdemon'){
	ini_set('error_reporting', E_ALL);
	ini_set('display_errors', 1);
	ini_set('display_startup_errors', 1);
...
// Output
/*if (isset($request->get['admin'])){
	echo $config->get('config_city');
	echo 'test';
}
$array_replace = array('в Москве'=>'в '.$config->get('config_city'));
$response->replace($array_replace);*/
$response->output();
