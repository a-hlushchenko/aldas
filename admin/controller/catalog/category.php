...
class ControllerCatalogCategory extends Controller {
    ...
    if ($this->config->get('config_editor_default')) {
		...
		// типы категорий
		if(isset($this->request->get['category_id'])) {
			//$data['types'] = $this->getTypes($this->data['parent_id']);
			$data['types'] = $this->getTypes($this->request->get['category_id']);
		} else {
			$data['types'] = array();
		}
		...
		if (isset($this->request->post['type_id'])) {
			...
			$data['type_id'] = $category_info['type_id'];
		} else {
			$data['type_id'] = 0;
		}
		...
		if (isset($this->request->post['view_attibute'])) {
			$data['view_attibute'] = $this->request->post['view_attibute'];
		} elseif (!empty($category_info)) {
			$data['view_attibute'] = $category_info['view_attibute'];
		} else {
			$data['view_attibute'] = 0;
		}
		if (isset($this->request->post['name_attibute'])) {
			$data['name_attibute'] = $this->request->post['name_attibute'];
		} elseif (!empty($category_info)) {
			$data['name_attibute'] = $category_info['name_attibute'];
		} else {
			$data['name_attibute'] = '';
		}
