...
class ModelNewsArticle extends Model {
	public function addArticle($data) {
		...
		if (isset($data['article_image'])) {
			foreach ($data['article_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "news_articles_image SET article_id = '" . (int)$article_id . "', image = '" . $this->db->escape($product_image['image']) . "', sort_order = '" . (int)$product_image['sort_order'] . "'");
			}
		}

		foreach ($data['article_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "news_articles_description SET article_id = '" . (int)$article_id . "', language_id = '" . (int)$language_id . "', 
				name = '" . $this->db->escape($value['name']) . "', 
				city = '" . $this->db->escape($value['city']) . "', 
				autor = '" . $this->db->escape($value['autor']) . "', 
				description = '" . $this->db->escape($value['description']) . "', short_description = '" . $this->db->escape($value['short_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', tag = '" . $this->db->escape($value['tag']) . "'");
		}
	...
	public function editArticle($article_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "news_articles_image WHERE article_id = '" . (int)$article_id . "'");
	...
	public function copyArticle($article_id){
		...
		if ($query->num_rows) {
			...
			$data['article_image'] = $this->getProductImages($article_id);
	...
	public function getProductImages($article_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news_articles_image WHERE article_id = '" . (int)$article_id . "' ORDER BY sort_order ASC");
