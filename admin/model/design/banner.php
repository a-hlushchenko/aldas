		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				...
				foreach ($banner_image['banner_image_description'] as $language_id => $banner_image_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image_description SET banner_image_id = '" . (int)$banner_image_id . "', language_id = '" . (int)$language_id . "', banner_id = '" . (int)$banner_id . "', title = '" .  $this->db->escape($banner_image_description['title']) . "', title2 = '" .  $this->db->escape($banner_image_description['title2']) . "', description = '" .  $this->db->escape($banner_image_description['description']) . "'");
				}
			}
		}
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			foreach ($banner_image_description_query->rows as $banner_image_description) {
				$banner_image_description_data[$banner_image_description['language_id']] = array('title' => $banner_image_description['title'],'title2' => $banner_image_description['title2'],'description' => $banner_image_description['description']);
			}
